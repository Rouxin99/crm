...
def save_final_suggestion(
    ...
    ai_flag = "優惠" if has_coupon else "無建議優惠券"

#寫入aisuggestion表
    obj = AiSuggection.objects.create(
        userID=str(user_id),
        categoryID=category_id,
        aiRecommendGuideline=ai_flag,
    ...
    # 先取 guideline 第一行當作優惠券名稱
    first_line = text.splitlines()[0].strip() if text else ""
    if not first_line:
        first_line = "未命名優惠券"

    coupon_type = first_line[:45] if first_line else "未命名優惠券"   # Campaign.type 最多 45 字

    Campaign.objects.create(
        customerid=None,      # 目前不綁特定顧客
        type=coupon_type,
        givetime=now,         # 發券時間 = 按下執行的時間
        starttime=start_dt,
        endtime=end_dt,
        isuse="0",
        suggectid=obj.suggectid,
    )
