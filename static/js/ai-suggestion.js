...
async function executeSuggestion(id) {
  ...
  const defaultEnd = toDateStr(new Date(today.getTime() + 30 * 24 * 60 * 60 * 1000));

  //讓使用者輸入開始/結束日期
  const startDate = window.prompt("請輸入優惠開始日期 (YYYY-MM-DD)：", defaultStart);
  if (startDate === null) {
    //使用者按取消
    return;
  }

  const endDate = window.prompt("請輸入優惠結束日期 (YYYY-MM-DD)：", defaultEnd);
  if (endDate === null) {
    return;
  }

  try {
    const resp = await fetch(API_EXECUTE, {
      method: "POST",
    ...
    if (!resp.ok) {
      alert("儲存失敗：" + (data.error || `HTTP ${resp.status}`));
      return;
    }
    ...
    alert(`已寫入資料庫，建議ID：${data.suggestID}`);

  } catch (err) {
    console.error("執行建議失敗", err);
    alert("執行建議時發生錯誤：" + err.message);
  }
}
